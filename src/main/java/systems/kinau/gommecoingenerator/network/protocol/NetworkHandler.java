...
package systems.kinau.gommecoingenerator.network.protocol;
...
import systems.kinau.gommecoingenerator.network.protocol.login.*;
...
public class NetworkHandler {
    ...
    public NetworkHandler() {
        try {
            this.out = new DataOutputStream(GommeCoinGenerator.getInstance().getSocket().getOutputStream());
            this.in = new DataInputStream(GommeCoinGenerator.getInstance().getSocket().getInputStream());

            this.state = State.HANDSHAKE;
            initPacketRegistries();
        } catch (IOException e) {
            GommeCoinGenerator.getLog().severe("Could not start bot: " + e.getMessage());
        }
    }
    ...
    private void initPacketRegistries() {
        ...
        getPlayRegistryOut().get(ProtocolConstants.MINECRAFT_1_12).registerPacket(0x08, PacketOutWindowClick.class);
        ...
        getPlayRegistryOut().get(ProtocolConstants.MINECRAFT_1_12).registerPacket(0x1F, PacketOutBlockPlacement.class);
        ...
        getPlayRegistryOut().get(ProtocolConstants.MINECRAFT_1_12_1).registerPacket(0x07, PacketOutWindowClick.class);
        ...
        getPlayRegistryOut().get(ProtocolConstants.MINECRAFT_1_12_1).registerPacket(0x1F, PacketOutBlockPlacement.class);
    ...
    public void sendPacket(Packet packet) {
        ...
        switch (getState()) {
            ...
            case LOGIN:
                ...
                break;
            case PLAY:
                Packet.writeVarInt(getPlayRegistryOut().get(GommeCoinGenerator.getInstance().getServerProtocol()).getId(packet.getClass()), buf);
                break;
        }

        //Add packet payload
        try {
            packet.write(buf, GommeCoinGenerator.getInstance().getServerProtocol());
        } catch (IOException e) {
            GommeCoinGenerator.getLog().warning("Could not instantiate " + packet.getClass().getSimpleName());
        }
        ...
        if (getThreshold() > 0) {
            ...
            try {
                ...
                GommeCoinGenerator.getLog().severe("Error while trying to send: " + packet.getClass().getSimpleName());
            }
        } else {
    ...
    private void readPacket(int len, int packetId, ByteArrayDataInputWrapper buf) throws IOException {
        ...
        switch (state) {
            ...
            case LOGIN:
                ...
                break;
            case PLAY:
                clazz = getPlayRegistryIn().get(GommeCoinGenerator.getInstance().getServerProtocol()).getPacket(packetId);
                break;
        }
...
//        GommeCoinGenerator.getLog().info("[" + getState().name().toUpperCase() + "] C <<< S: " + clazz.getSimpleName());

        try {
            Packet packet = clazz.newInstance();
            packet.read(buf, this, len, GommeCoinGenerator.getInstance().getServerProtocol());
        } catch (InstantiationException | IllegalAccessException e) {
            GommeCoinGenerator.getLog().warning("Could not create new instance of " + clazz.getSimpleName());
            e.printStackTrace();
        }
    }
    ...
    public void activateEncryption() {
        try {
            ...
            BufferedOutputStream var1 = new BufferedOutputStream(CryptManager.encryptOuputStream(getSecretKey(), GommeCoinGenerator.getInstance().getSocket().getOutputStream()), 5120);
    ...
    public void decryptInputStream() {
        ...
        try {
            InputStream var1;
            var1 = GommeCoinGenerator.getInstance().getSocket().getInputStream();
