...
package systems.kinau.gommecoingenerator.auth;
...
public class Authenticator {
    ...
    public AuthData authenticate() {
        ...
        try {
            ...
            GommeCoinGenerator.getLog().info("Try to authenticate " + username + ":" + password.replaceAll(".", "*"));

            if (answer.getStatusLine().getStatusCode() != 200) {
                GommeCoinGenerator.getLog().severe("Authentication failed with code " + answer.getStatusLine().getStatusCode() + " from " + AUTH_SERVER + ": " + answer.getStatusLine());
                GommeCoinGenerator.getLog().severe(EntityUtils.toString(answer.getEntity(), Charsets.UTF_8));
                return null;
            }
            ...
            GommeCoinGenerator.getLog().info("Authentication successful!");
            return new AuthData(accessToken, clientToken, profile, username);
        } catch (IOException e) {
            GommeCoinGenerator.getLog().severe("Error while connecting to: " + AUTH_SERVER);
        }
        return null;
    }
}
